    ...
    <div class="mt-2 mb-2 col-lg-12">
        <div class="d-flex justify-content-end">
            <a href="{{ route('admin.city_add_form') }}">
                <button class="btn btn-add px-4" type="button">Add Destination</button>
            </a>
        </div>
    </div>
...
<img
src="{{ asset($row->cities_pic) }}"
alt="{{ $row->alternate_name ?? 'Default Alt Text' }}"
style="max-width:56px;max-height:56px;object-fit:cover;">
...
data-row_id="{{ $row->id }}"
data-act_url="{{ route('admin.city_status') }}"
data-stsmode="{{ $mode }}">
...
<a href="{{ route('admin.city_edit_form',$row->id) }}"
title="Edit"
...
title="Delete"
data-row_id="{{ $row->id }}"
data-act_url="{{ route('admin.city_delete') }}"
data-csrf_token="{{ csrf_token() }}">
