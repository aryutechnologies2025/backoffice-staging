    ...
    <div class="col-lg-12">
        <div class="form-body px-4 mb-5 ms-4 me-5 rounded-4">
                    ...
                    <div class="row gap-2">
                        ...
                        <div class="add_form col-md-4 ">
                            ...
                            <select id="cities_name" name="cities_name"
                                ...
                                <option value="{{ $id }}"
                                    @if(old('cities_name', $stay_details->destination ?? '') == $id) selected @endif>
                                    {{ $name }}
                                </option>
                                @endforeach
                            </select>
            ...
            <div class="form-input">
                ...
                <input type="file" name="new_images[]" id="file-ip-${photoCount}" data-number="${photoCount}" accept="image/png, image/jpeg, image/svg+xml" onchange="previewImage(event, this)">
...
<script>
    $(document).ready(function() {
        ...
        $('form').on('submit', function() {
         ...
         $(document).on('change', '#cities_name', function() {
            ...
            // AJAX request
            $.ajax({
                url: '{{ route('stay_list.getdistrictslist') }}', // Fixed route syntax
                type: 'GET',
                data: {
                ...
                success: function(data) {
                    ...
                    if (data && data.length > 0) {
                        ...
                        // For edit page: Set the previously selected district
                        const previouslySelectedDistrict = '{{ $stayList->district_name ?? '' }}';
                        if (previouslySelectedDistrict) {
                            districtSelect.val(previouslySelectedDistrict);
                        }
        ...
        // // If we're in edit mode and a city is already selected
        // const selectedCity = '{{ $stay_details->destination ?? '' }}'; 
        // if (selectedCity) {
        //     console.log('Edit mode detected, selected city:', selectedCity);
        //     // Set the city dropdown value
        //     $('#cities_name').val(selectedCity);
