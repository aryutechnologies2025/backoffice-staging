...
<td class="text-start">
{{ optional($row->stag)->stay_title ?? 'N/A' }}
</td>
...
<td class="text-start">
{{ $row->created_at ? $row->created_at->timezone('Asia/Kolkata')->format('d-m-Y h:i:s A') : '' }}
</td>
...
data-row_id="{{ $row->id }}"
data-act_url="{{ route('admin.stay_review_status') }}"
data-stsmode="{{ $mode }}">
...
<a href="{{ route('admin.stay_review_edit_form',$row->id) }}"
...
data-row_id="{{ $row->id }}"
data-act_url="{{ route('admin.stay_review_delete') }}"
data-csrf_token="{{ csrf_token() }}">

<span class="fa-stack">
<i class="fa fa-trash-o fa-stack-1x fa-inverse" style="color:red"></i>
</span>
