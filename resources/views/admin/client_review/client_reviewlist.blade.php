...
<div class="row body-sec py-3 px-5 justify-content-around">
    ...
    <div class="mt-2 mb-2 col-lg-12">
        <div class="d-flex justify-content-end">
             @if(\App\Helpers\PermissionHelper::has($permissions, 'review', 'create'))
...
{{ $row->created_at->timezone('Asia/Kolkata')->format('d-m-Y h:i:s A') }}
                        ...
                        @endphp
                        <td class="text-start"><a data-toggle="tooltip" data-csrf_token="{{ csrf_token() }}" data-original-title="{{ $actTitle }}" class="stsconfirm" href="javascript:void(0);" data-row_id="{{ $row->id }}" data-act_url="{{ route('admin.client_review_status') }}" data-stsmode="{{ $mode }}"><button type="button" class="btn {{ $btnColr }} px-5">{{ $disp_status }}</button></a></td>
                        <td class="text-start" style="width: 20%;">
                            <a href="{{ route('admin.client_review_edit_form',$row->id) }}" title="Edit" class="table-edit-link">
                                <span class="fa-stack">
                            ...
                            <a href="javascript:void(0);" class="table-link danger delconfirm" title="Delete" data-row_id="{{ $row->id }}" data-act_url="{{ route('admin.client_review_delete') }}" data-csrf_token="{{ csrf_token() }}">
                                <span class="fa-stack">
                                    <!-- <i class="fa fa-square fa-stack-2x"></i> -->
