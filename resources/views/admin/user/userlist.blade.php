                ...
                <table id="cityTable" class="table table-bordered pt-2">
                    <thead>
                        <tr class="rounded-top-4">
                            ...
                            <th class="text-start"><span>Address</span></th>
                        ...
                        @else
                            @foreach ($user_dts as $row)
                                <tr>
                                    ...
                                    <td class="text-start px-3">{{ $row->first_name }} {{ $row->last_name }}</td>
                                    <td class="text-start px-2">{{ $row->email }}</td>
                                    <!-- <td class="text-center w-45">{{ $row->dob }}</td> -->
                                    <td class="text-start px-3">{{ $row->phone }}</td>
                                    ...
                                    @endphp
                                    <td class="text-start"><a data-csrf_token="{{ csrf_token() }}"
                                            data-original-title="{{ $actTitle }}" class="stsconfirm"
                                            href="javascript:void(0);" data-row_id="{{ $row->id }}"
                                            data-act_url="{{ route('admin.user_status') }}"
                                            data-stsmode="{{ $mode }}"><button type="button"
                                                class="btn {{ $btnColr }} px-5">{{ $disp_status }}</button></a></td>
                                    <td class="text-start" style="width: 20%;">
                                        <a href="{{ route('admin.user_view_form', $row->id) }}"
                                            title="View" class="table-edit-link">
                                            <span class="fa-stack">
                                                <i class="fa-regular fa fa-eye fa-stack-1x fa-inverse"
                                                    style="color:blue !important;"></i>
                                            </span>
                                        </a>
                                        <a href="{{ route('admin.user_edit_form', $row->id) }}"
                                            title="Edit" class="table-edit-link">
                                            <span class="fa-stack">
                                        ...
                                        <a href="javascript:void(0);" title="Delete"
                                            class="table-link danger delconfirm" data-row_id="{{ $row->id }}"
